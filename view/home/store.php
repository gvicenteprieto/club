<?php
    ...
    if (empty($dni) || empty($email) || empty($usuario) || empty($apellidos) || empty($nombres) || empty($pass) || empty($confirmPass)) {
        ...
        header("Location:panelControl.php?error=" . $error . "&&dni=" . $dni. "&&usuario=" . $usuario . "&&apellidos=" . $apellidos . "&&nombres=" 
        . $nombres . "&&email=" . $email . "&&pass=" . $pass . "&&confirmPass=" . $confirmPass);
    } else {
        //verificando que las contraseñas coincidan
        if ($confirmPass == $pass) {
            //si coinciden, se reutiliza el objeto que almacena el home controller para acceder a la función guardarUsuario() y se dirige a login; 
            //aunque hay otra valicación más ya que el correo debe ser único
            if ($obj->guardarUsuario($dni, $usuario, $apellidos, $nombres, $email, $pass, $foto) == false) {
                ...
                header("Location:panelControl.php?error=" . $error . "&&dni=" . $dni."&&usuario=" . $usuario . "&&apellidos=" . $apellidos . "&&nombres=" 
                . $nombres . "&&email=" . $email . "&&pass=" . $pass . "&&foto=" . $foto ."&&confirmPass=" . $confirmPass);
            } else {
